...
## Resets game state and pauses the game by setting the time scale to 0
func _setup():
	game_started = false
	is_game_over = false
	_init_target_score()
...
## Called at the start of the game session; spawns the first food
func _start():
	_spawn_food()
	
## Handles the first key press to begin the game (ignores held keys and repeats)
...
## Main game loop tick: updates grid, handles movement, checks for collisions and win/loss conditions
func _tick(float):
	_clear_grid()
	_draw_food()
	snake.move_snake()
...
## Spawns food at a random empty location on the grid
func _spawn_food():
...
## Checks whether the snake's head has gone out of the play area bounds
...
## Ends the game, disables processing, and emits the game_over signal
func _game_over():
	is_game_over = true
...
## Returns true if the score has reached the set target
...
## Initializes the score target based on selected game mode
...
## Prevents the snake from turning directly back onto itself
...
## Handles user input for reloading the game or exiting to the main menu
func _input(event):
...
## Reloads the game if it's currently over
...
## Returns to main menu, only allowed after the game is over
func _main_menu():
	if not is_game_over:
		return
